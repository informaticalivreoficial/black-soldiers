const mix = require('laravel-mix');


mix
 //WEB
 .styles([
     'resources/views/web/css/style.css'
 ], 'public/frontend/assets/css/style.css') 
  
 .styles([
     'resources/views/web/css/bootstrap.min.css'
 ], 'public/frontend/assets/css/bootstrap.min.css') 
  
 .styles([
     'resources/views/web/css/font-awesome.min.css'
 ], 'public/frontend/assets/css/font-awesome.min.css') 
  
 .styles([
     'resources/views/web/css/owl.carousel.min.css'
 ], 'public/frontend/assets/css/owl.carousel.min.css') 
  
 .styles([
     'resources/views/web/css/magnific-popup.css'
 ], 'public/frontend/assets/css/magnific-popup.css') 
  
 .styles([
     'resources/views/web/css/animate.css'
 ], 'public/frontend/assets/css/animate.css') 
  
 .styles([
     'resources/views/web/css/main.css'
 ], 'public/frontend/assets/css/main.css') 
  
 .styles([
     'resources/views/web/css/meanmenu.min.css'
 ], 'public/frontend/assets/css/meanmenu.min.css') 
  
 .styles([
     'resources/views/web/css/responsive.css'
 ], 'public/frontend/assets/css/responsive.css') 
  
.copyDirectory('resources/views/web/fonts','public/frontend/assets/fonts')
//.copyDirectory('resources/views/web/images','public/frontend/assets/images')

.scripts([
    'resources/views/web/js/jquery-2.2.4.min.js'
], 'public/frontend/assets/js/jquery-2.2.4.min.js')

.scripts([
    'resources/views/web/js/bootstrap.min.js'
], 'public/frontend/assets/js/bootstrap.min.js')
 
.scripts([
    'resources/views/web/js/imagesloaded.pkgd.min.js'
], 'public/frontend/assets/js/imagesloaded.pkgd.min.js')
 
.scripts([
    'resources/views/web/js/isotope.pkgd.min.js'
], 'public/frontend/assets/js/isotope.pkgd.min.js')
 
.scripts([
    'resources/views/web/js/owl.carousel.min.js'
], 'public/frontend/assets/js/owl.carousel.min.js')
 
.scripts([
    'resources/views/web/js/owl.animate.js'
], 'public/frontend/assets/js/owl.animate.js')
 
.scripts([
    'resources/views/web/js/jquery.scrollUp.min.js'
], 'public/frontend/assets/js/jquery.scrollUp.min.js')
 
.scripts([
    'resources/views/web/js/jquery.counterup.min.js'
], 'public/frontend/assets/js/jquery.counterup.min.js')
 
.scripts([
    'resources/views/web/js/modernizr.min.js'
], 'public/frontend/assets/js/modernizr.min.js')

.scripts([
    'resources/views/web/js/jquery.magnific-popup.min.js'
], 'public/frontend/assets/js/jquery.magnific-popup.min.js')

.scripts([
    'resources/views/web/js/wow.min.js'
], 'public/frontend/assets/js/wow.min.js')

.scripts([
    'resources/views/web/js/waypoints.min.js'
], 'public/frontend/assets/js/waypoints.min.js')

.scripts([
    'resources/views/web/js/jquery.meanmenu.min.js'
], 'public/frontend/assets/js/jquery.meanmenu.min.js')

.scripts([
    'resources/views/web/js/jquery.sticky.js'
], 'public/frontend/assets/js/jquery.sticky.js')

.scripts([
    'resources/views/web/js/custom.js'
], 'public/frontend/assets/js/custom.js')
 
 .options({
     processCssUrls: false
 })
 
 .version()
     
;